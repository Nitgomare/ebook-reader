...
| 算法 | 原理 | 特点 | ROS2包 |
| --- | --- | --- | --- |
| GMapping | 粒子滤波（Rao-Blackwellized） | 经典算法，适合小环境，计算量较大 | slam_gmapping |
| Cartographer | 图优化（Google开源） | 大场景效果好，支持回环检测，计算量中等 | cartographer_ros |
| Karto SLAM | 图优化 | 计算效率高，适合中等环境 | slam_karto |
| Hector SLAM | 扫描匹配（高斯牛顿） | 无需里程计，高速激光，适合无人机 | hector_slam |
| Slam Toolbox | 图优化+ lifelong mapping | ROS2推荐，支持地图合并、持续建图 | slam_toolbox |
...
+ 分辨率：通常0.05m/格（5cm）
+ 格式：PGM图像文件（灰度图，黑色=占用，白色=自由，灰色=未知）+ YAML配置文件
...
ROS2 Nav2默认使用NavFn（A_的一种实现）或Smac Planner（支持A_、Hybrid A*、State Lattice）。
...
```plain
score = α × heading(θ) + β × dist(obstacle) + γ × velocity
```
...
DWA的参数（α, β, γ, 速度限制、加速度限制、模拟时间等）需要根据机器人和环境调优。
...
_图15-1：RViz中显示的SLAM导航代价地图，灰色为未知，黑色为障碍物，蓝色为膨胀层，红色/绿色为机器人和路径_
...
_图15-2：代价地图膨胀层示意图，距离机器人中心越近代价越高，致命障碍=254，内切障碍=253，膨胀层=128~252，自由空间=0_
...
OpenManipulator是ROBOTIS出品的开源机械臂，与TurtleBot3配合使用，基于Dynamixel舵机，支持MoveIt2。
...
class MoveIt2Demo(Node):
    ...
    def go_to_pose(self, x, y, z, w=1.0):
        ...
        self.arm.set_goal_state(pose_stamped_msg=pose, pose_link="end_effector_link")
...
# 启动MoveIt2仿真
ros2 launch open_manipulator_x_moveit_config moveit_planning_execution.launch.py sim:=True
...
> **MoveIt 2 — ROS2机械臂运动规划框架**  
MoveIt2官方仓库，包含运动学（KDL/IKFast/TRAC-IK）、运动规划（OMPL/CHOMP/STOMP）、碰撞检测（FCL）、轨迹处理、抓取规划等完整功能，支持多种机械臂，附Setup Assistant配置工具和Python/C++ API。  
