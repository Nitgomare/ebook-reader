...
| 型号 | 类型 | 范围 | 角度 | 频率 | 价格 |
| --- | --- | --- | --- | --- | --- |
| RPLIDAR A1 | 2D | 12m | 360° | 5.5Hz | 低 |
| RPLIDAR A2 | 2D | 18m | 360° | 10Hz | 低 |
| RPLIDAR S1 | 2D | 40m | 360° | 10Hz | 中 |
| Hokuyo UST-10LX | 2D | 10m | 270° | 40Hz | 中高 |
| Hokuyo UTM-30LX | 2D | 30m | 270° | 40Hz | 高 |
| Velodyne VLP-16 | 3D | 100m | 360° | 10Hz | 很高 |
| Ouster OS1-64 | 3D | 120m | 360° | 10/20Hz | 很高 |
| RoboSense RS-LiDAR-16 | 3D | 150m | 360° | 10Hz | 高 |
...
```plain
std_msgs/Header header    # 时间戳和坐标系（frame_id通常为laser_link或base_scan）
float32 angle_min          # 起始角度（rad，通常为-π）
float32 angle_max          # 结束角度（rad，通常为π）
float32 angle_increment    # 角度增量（rad，相邻两束激光的角度差）
float32 time_increment     # 测量时间增量（s，相邻两束激光的时间差）
float32 scan_time          # 扫描时间（s，一整圈扫描的时间）
float32 range_min          # 最小测量距离（m）
float32 range_max          # 最大测量距离（m）
float32[] ranges           # 距离数据数组（长度 = (angle_max-angle_min)/angle_increment + 1）
float32[] intensities      # 反射强度数组（可选，部分雷达支持）
```
...
# 查看话题
ros2 topic list  # /camera/color/image_raw, /camera/depth/image_rect_raw, /camera/imu等
```
...
+ **加速度计（Accelerometer）**：测量三轴加速度（包括重力加速度），单位m/s²
...
```plain
std_msgs/Header header
geometry_msgs/Quaternion orientation          # 姿态四元数（如果IMU自带姿态估计）
float64[9] orientation_covariance             # 姿态协方差矩阵（行优先）
...
分辨率用PPR（Pulses Per Revolution，每转脉冲数）表示，如400PPR、1024PPR。通过四倍频（A/B相的上升沿和下降沿），实际分辨率为4×PPR。

### 13.5.2 电机控制
电机控制通常采用三级PID闭环：

```plain
位置环（外环）──► 速度环（中环）──► 电流环（内环）──► 电机
   PID                PID                PID
```
...
def calculate_odometry(left_ticks, right_ticks, prev_left, prev_right, 
    ...
    """
    # 计算轮速增量
    delta_left = 2 * math.pi * wheel_radius * (left_ticks - prev_left) / ticks_per_rev
    delta_right = 2 * math.pi * wheel_radius * (right_ticks - prev_right) / ticks_per_rev
    
    # 差速机器人运动学
    delta_dist = (delta_left + delta_right) / 2.0  # 中心移动距离
    delta_theta = (delta_right - delta_left) / wheel_separation  # 航向角变化
...
ros2 run serial_motor_demo driver --ros-args -p serial_port:=/dev/ttyACM0 -p baud_rate:=115200
```
