...
+ **连杆扭转角（α_i）**：相邻两关节轴线之间的夹角。绕公垂线a_i旋转，从关节i轴线转到关节i+1轴线的角度。
...
| 参数 | 符号 | 描述 | 转动关节 | 移动关节 |
| --- | --- | --- | --- | --- |
| 连杆长度 | a_i | 关节轴i和i+1之间的距离 | 常数 | 常数 |
| 连杆扭转 | α_i | 关节轴i和i+1之间的夹角 | 常数 | 常数 |
| 连杆偏移 | d_i | 沿关节轴i，两公垂线之间的距离 | 常数 | 变量 |
| 关节角 | θ_i | 绕关节轴i，两公垂线之间的夹角 | 变量 | 常数 |
...
Denavit-Hartenberg（D-H）参数法是描述操作臂连杆坐标系的标准方法，由Denavit和Hartenberg于1955年提出。通过在每个连杆上建立坐标系，用四个参数（a_i, α_i, d_i, θ_i）确定相邻连杆坐标系之间的变换。

坐标系建立规则（标准D-H）：

1. **z轴**：z_{i-1}轴沿关节i的运动轴线方向
2. **x轴**：x_i轴沿z_{i-1}和z_i的公垂线方向，从z_{i-1}指向z_i
3. **原点**：O_i在z_i轴与x_i轴的交点
...
_图3-1：D-H参数连杆坐标系设定，a_j为连杆长度，α_j为连杆扭转角，d_j为连杆偏移，θ_j为关节角_

### 3.3.2 D-H变换矩阵
连杆i坐标系{i}相对于连杆i-1坐标系{i-1}的变换由四个基本变换按顺序组成：

```plain
^{i-1}T_i = Rotz(θ_i) × Transz(d_i) × Transx(a_i) × Rotx(α_i)
```
...
| 关节i | a_i (m) | α_i (rad) | d_i (m) | θ_i (rad) | 类型 |
| --- | --- | --- | --- | --- | --- |
| 1 | 0 | -π/2 | 0.6718 | θ_1 | 转动 |
| 2 | 0.4318 | 0 | 0 | θ_2 | 转动 |
| 3 | 0.0203 | -π/2 | 0.15005 | θ_3 | 转动 |
| 4 | 0 | π/2 | 0.4318 | θ_4 | 转动 |
| 5 | 0 | -π/2 | 0 | θ_5 | 转动 |
| 6 | 0 | 0 | 0 | θ_6 | 转动 |
...
_图3-2：五自由度机械臂的D-H坐标系建立，每个关节处建立坐标系x_i-y_i-z_i_
...
```plain
^0 T_n = ^0 T_1 × ^1 T_2 × ^2 T_3 × ... × ^{n-1}T_n
```

这个乘积的结果是一个4×4的齐次变换矩阵，包含末端的位置（最后一列前3个元素）和姿态（左上角3×3矩阵）。
...
dh_params = [
    {'a': l1, 'alpha': 0, 'd': 0, 'theta': theta1},
    {'a': l2, 'alpha': 0, 'd': 0, 'theta': theta2},
...
| 构型 | 关节类型 | 工作空间 | 典型应用 |
| --- | --- | --- | --- |
| 笛卡尔（Cartesian） | 3移动 | 长方体 | 龙门式、3D打印 |
| 圆柱（Cylindrical） | 1转动+2移动 | 圆柱体 | 搬运、装配 |
| 球坐标（Spherical） | 2转动+1移动 | 球体 | 焊接、喷涂 |
| 关节型（Articulated） | 3转动 | 类球体 | 通用工业机械臂 |
| SCARA | 2转动+1移动 | 平面 | 电子装配 |
