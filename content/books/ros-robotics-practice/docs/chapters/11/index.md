...
# 发送动作目标并查看反馈
ros2 action send_goal --feedback /action_name action_type "{...}"
```
...
### 11.9.1 RViz简介
RViz是ROS的三维可视化工具，可以显示机器人模型、传感器数据、地图、路径、TF坐标系等。它是ROS开发中最重要的调试和可视化工具。
...
_图11-1：RViz可视化界面，显示机器人模型、TF坐标系、激光雷达数据、里程计轨迹等_
...
# Node(package='rviz2', executable='rviz2', arguments=['-d', config_path])
```
...
| Display | 功能 | 常用话题 |
| --- | --- | --- |
| RobotModel | 显示机器人URDF模型 | /robot_description（参数） |
| LaserScan | 显示二维激光雷达 | /scan |
| PointCloud2 | 显示三维点云 | /points2 |
| Image | 显示相机图像 | /camera/image_raw |
| Map | 显示二维栅格地图 | /map |
| Path | 显示路径 | /path |
| TF | 显示坐标系变换 | /tf, /tf_static |
| Marker | 显示自定义标记 | /marker |
| Odometry | 显示里程计轨迹 | /odom |
| PoseArray | 显示位姿数组（粒子滤波） | /particlecloud |
| Polygon | 显示多边形 | /footprint |
...
| 插件 | 命令 | 功能 |
| --- | --- | --- |
| rqt_graph | `rqt_graph` | 显示节点和话题的通信关系图 |
| rqt_plot | `rqt_plot` | 绘制话题数据的实时曲线 |
| rqt_console | `rqt_console` | 日志消息查看器（过滤、分级显示） |
| rqt_reconfigure | `rqt_reconfigure` | 动态参数配置（实时修改参数） |
| rqt_image_view | `rqt_image_view` | 图像查看器（选择话题显示图像） |
| rqt_bag | `rqt_bag` | rosbag文件查看和编辑 |
| rqt_tf_tree | `rqt_tf_tree` | TF坐标系树可视化 |
| rqt_publisher | `rqt_publisher` | 图形化消息发布器 |
| rqt_service_caller | `rqt_service_caller` | 图形化服务调用器 |
| rqt_shell | `rqt_shell` | 嵌入式终端 |
...
# 示例：绘制激光雷达前三个距离值
rqt_plot /scan/ranges[0] /scan/ranges[1] /scan/ranges[2]
```
...
详细演示RViz2配置、Gazebo仿真环境搭建、rqt工具箱使用（rqt_graph、rqt_plot、rqt_console）和rosbag数据录制回放，是ROS2调试必备技能。  
