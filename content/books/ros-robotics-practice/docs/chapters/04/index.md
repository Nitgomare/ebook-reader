...
由余弦定理，末端到基座的距离 r = √(x² + y²)：

```plain
r² = l1² + l2² - 2 l1 l2 cos(π - θ2)
...
+ |cos(θ2)| ≤ 1，即 |r² - l1² - l2²| ≤ 2 l1 l2
+ 等价于 |l1 - l2| ≤ r ≤ l1 + l2

### 4.3.2 三自由度平面臂的逆运动学
三自由度平面臂可以到达任意位姿(x, y, φ)。由于多了一个自由度，可以先指定腕部位置，再求解。

一种常用方法：

1. 给定末端位姿(x, y, φ)，指定腕部位置偏移：(x_w, y_w) = (x - l3 cos(φ), y - l3 sin(φ))
2. 对前两个连杆用二连杆逆运动学求解θ1, θ2
3. θ3 = φ - θ1 - θ2
...
1. **位置逆解**：已知腕部中心位置，求解前3个关节（θ1, θ2, θ3）
...
def inverse_kinematics_dls(target, q0, l1, l2, max_iter=100, tol=1e-6, lam=0.1):
    """阻尼最小二乘法（DLS）求解逆运动学"""
...
其中J<sup>+是伪逆，q̇_0是次要目标的关节速度，(I - J</sup>+ J)是零空间投影矩阵。

### 📺 推荐视频
> **ROS2 MoveIt 2机械臂控制实战**  
