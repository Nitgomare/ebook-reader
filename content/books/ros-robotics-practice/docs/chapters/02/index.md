...
_图2-1：移动机器人在空间中的位姿描述，包含位置向量p和旋转矩阵R组成的齐次变换矩阵T_

## 2.2 描述：位置、姿态与位姿
<!-- 这是一张图片，ocr 内容为： -->
![24_277_168_1211_840_0.jpg](../../images/24_277_168_1211_840_0.jpg)

_图2-3 旋转表示方法对比（旋转矩阵/欧拉角/四元数）_
...
设刚体上固连坐标系{B}，其三个单位主轴向量为 <sup>A X_B、</sup>A Y_B、^A Z_B（在参考系{A}中表示），则旋转矩阵为：

```plain
^A R_B = [^A X_B, ^A Y_B, ^A Z_B] = [r11 r12 r13]
                                       [r21 r22 r23]
                                       [r31 r32 r33]
```
...
+ R^T R = I（正交性）
+ det(R) = 1（行列式为1，保证右手坐标系）
+ R<sup>{-1} = R</sup>T（逆等于转置）

旋转矩阵的9个元素中只有3个是独立的，因为满足6个约束条件（3个列向量单位正交 + 行列式=1）。

<!-- 这是一张图片，ocr 内容为： -->

_图2-2：坐标系{B}相对于{V}旋转角度θ，点P在两个坐标系下的坐标通过旋转矩阵联系_
...
欧拉角的缺点是存在"万向节锁"（Gimbal Lock）问题，当Pitch角为±90°时，Roll和Yaw轴重合，失去一个自由度。
    ...
    - Pitch（俯仰角θ）：绕Y轴旋转
    - Yaw（偏航角ψ）：绕Z轴旋转
2. **固定角（Fixed Angles）**：绕固定坐标系的轴旋转，与欧拉角的旋转顺序相反。XYZ固定角等价于ZYX欧拉角。
3. **角-轴表示（Angle-Axis）**：用一个单位向量k（旋转轴）和一个角度θ（绕轴旋转量）描述旋转。
4. **四元数（Quaternion）**：用四个参数 [w, x, y, z] 描述旋转，其中w是实部，x、y、z是虚部。四元数避免了万向节锁，计算效率高，是ROS中默认的姿态表示方式。单位四元数满足 w² + x² + y² + z² = 1。
...
+ 最后一行 [0 0 0 1]：齐次坐标的标准形式

齐次变换矩阵的优点：可以用矩阵乘法连续表示多次变换，形式简洁统一。在机器人学中，齐次变换矩阵是描述连杆坐标系之间关系的标准工具。

## 2.3 映射：从一个坐标系到另一个坐标系的变换
### 2.3.1 平移映射
已知点P在坐标系{B}中的位置 ^B P，且{B}的原点在{A}中的位置为 ^A P_Borg，两坐标系姿态相同，则P在{A}中的位置为：
...
### 2.3.2 旋转映射
已知点P在坐标系{B}中的位置 ^B P，两坐标系原点重合，{B}相对于{A}的旋转矩阵为 ^A R_B，则P在{A}中的位置为：
...
_图2-3：绕Y轴旋转角度β，x轴和z轴在x-z平面内旋转，y轴保持不变_
...
注意：旋转矩阵的乘法顺序很重要，不满足交换律，即 R1×R2 ≠ R2×R1。
...
### 2.5.2 逆变换
已知 ^A T_B，则 ^B T_A（B到A的逆变换）为：

```plain
^B T_A = (^A T_B)^(-1) = [R^T  -R^T P]
                           [0     1    ]
```
...
### 2.5.3 ROS中的TF变换系统
ROS（机器人操作系统）提供了TF（Transform）库，用于管理和查询坐标系之间的变换关系。TF2是ROS2中的升级版本。
