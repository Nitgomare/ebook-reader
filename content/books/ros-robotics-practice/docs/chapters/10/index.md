...
图10-1:ROS话题通信模型，包含ROS Master(ROS1)、Talker(发布者)和Listener(订阅者)，通过注册、匹配、连接、数据传输的流程实现通信
...
private:
	...
	void timer_callback() {
		...
		RCLCPP_INFO(this->get_logger(), "Publishing: '%s'", message.data.c
...
Syntnoic Publisl Communication Mode
...
2 ros2 service type /service_name # 查看服务类型

3 * ros2 service call /service_name type "\{request\}" # 调用服务

---

**话题 vs 服务对比:**

<table id="cross-table-2"><tr><td>特性</td><td>话题 (Topic)</td><td>服务(Service)</td></tr><tr><td>模式</td><td>发布/订阅</td><td>请求/响应</td></tr><tr><td>通信</td><td>异步</td><td>同步</td></tr><tr><td>连接</td><td>多对多</td><td>一对多(多客户端)</td></tr><tr><td>数据</td><td>持续流</td><td>一次性</td></tr><tr><td>适用</td><td>传感器数据、状态</td><td>查询、触发操作</td></tr><tr><td>反馈</td><td>无</td><td>有响应</td></tr></table>

### 10.2.3 动作 (Action)

动作是ROS2中较复杂的通信方式，适合长时间运行的任务:
...
- 每个节点有自己的参数(参数属于节点，不是全局共享的，这是R0S2与R0S1的区别)
...
ros2 param load /node_name 					ile.yaml # 从YAML文件加载参数
...
.action文件定义动作的目标、结果和反馈，用 ___ 分隔:
...
TF(Transform)是ROS中管理坐标系变换的系统，用于跟踪和查询多个坐标系之间的关系。TF2是 ROS2中的升级版本。
...
ros2 pkg create --build-type ament_cmake --dependencies rclcpp std_msgs my_
...
3 colcon build --symlink-install 																																											#符号链接安装(开发时用，修改Python脚本无

				需重新编译)

4 colcon build --cmake-args -DCMAKE_BUILD_TYPE=Debug # Debug模式

			colcon test 																																											#运行测试

6 colcon test-result --verbose 																																										#查看测试结果
