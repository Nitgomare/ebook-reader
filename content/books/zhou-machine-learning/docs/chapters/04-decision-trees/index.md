...
!!! success "把“最优划分”放进机械表格特征"
    前往 [树与集成实验](/programming/ml-from-scratch/#tree-ensemble-lab)，理解回归树如何最小化两侧平方误差，以及真正的随机森林为什么同时需要样本随机性和特征随机性。随后在 [完整项目练习](/programming/first-project/) 中与逻辑回归做基线对照。

延伸源码：[tiny_ml 决策树实现](https://github.com/fengyang95/tiny_ml/tree/master/tinyml/tree)。阅读时重点检查停止条件、空分支、叶节点样本数和预测遍历。
