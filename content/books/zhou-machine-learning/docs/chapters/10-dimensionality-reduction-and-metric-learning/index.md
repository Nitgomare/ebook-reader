...
## 本站实践：PCA 图上的分离可能只是工况

!!! success "训练集拟合，测试集只变换"
    [PCA NumPy 实验](/programming/ml-from-scratch/#pca-lab) 展示中心化、协方差特征分解和投影，并用 scikit-learn `Pipeline` 防止测试集信息泄漏。高方差方向不一定与预测目标相关，因此需要结合原始变量解释主成分。

延伸源码：[tiny_ml 降维实现](https://github.com/fengyang95/tiny_ml/tree/master/tinyml/dimension_reduction)，还可比较 Kernel PCA、LLE、Isomap 与 MDS 的假设。
