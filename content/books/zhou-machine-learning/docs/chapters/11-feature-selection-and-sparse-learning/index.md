...
!!! warning "先选择特征、再划分数据，会泄漏标签信息"
    特征选择、标准化与 PCA 都应在训练集内部拟合，并与模型一起放进 Pipeline。使用 [源码阅读检查表](/programming/ml-from-scratch/#validation-lab) 审查数据流，再到 [完整项目](/programming/first-project/) 做按批次分组的验证。

延伸源码：[tiny_ml Relief 特征选择](https://github.com/fengyang95/tiny_ml/blob/master/tinyml/feature_selection/ReliefFeatureSelection.py)。重点检查近邻搜索是否只使用训练数据，以及特征尺度对距离的影响。
