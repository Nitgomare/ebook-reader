...
!!! success "读完公式，马上做一次数值对照"
    在 [线性回归 NumPy 实验](/programming/ml-from-scratch/#linear-lab) 中，你会将闭式解改写为数值更稳定的 `np.linalg.lstsq`，再与 scikit-learn 的 OLS、Ridge 使用同一划分比较。重点不是复刻 API，而是检查截距、矩阵秩、特征尺度与误差指标。

延伸源码：[tiny_ml 线性模型实现](https://github.com/fengyang95/tiny_ml/tree/master/tinyml/linear_model)。建议先独立完成实验，再阅读上游实现并用本站检查表做代码审阅。
