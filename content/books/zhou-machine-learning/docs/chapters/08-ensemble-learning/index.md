...
!!! success "检查弱学习器如何产生差异"
    [树与集成实验](/programming/ml-from-scratch/#tree-ensemble-lab) 会拆开 bootstrap、随机特征与预测平均三个环节，并指出只有随机特征、没有 bootstrap 的实现为何不等同于完整随机森林。之后在 [完整项目练习](/programming/first-project/) 中运行 300 棵树的可复现基线。

延伸源码：[tiny_ml 集成学习实现](https://github.com/fengyang95/tiny_ml/tree/master/tinyml/ensemble)，包括 AdaBoost、GBDT、随机森林和简化 XGBoost。
