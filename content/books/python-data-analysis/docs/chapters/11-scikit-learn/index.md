...
## 本站实践：从 API 入门到可信机械实验

本章帮助你认识 scikit-learn 的常用接口；在真实项目中，还需要补上数据分组、Pipeline、基线比较和误差分析。

!!! success "下一步不是换更复杂的模型"
    先运行本站的 [第一个机器学习完整项目](/programming/first-project/)。它使用一组自带标签的合成信号演示特征提取、按 `run_id` 分组划分、逻辑回归与随机森林对照，并自动保存指标和混淆矩阵。

如果想理解 `fit()` 内部发生了什么，进入 [从零读懂机器学习](/programming/ml-from-scratch/)，用 NumPy 复现线性回归、K-Means 和 PCA 的关键步骤，再与 scikit-learn 对照。
