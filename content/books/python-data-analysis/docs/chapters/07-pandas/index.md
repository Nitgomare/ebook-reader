...
!!! info "本章补充视频（非教材配套）"
    以下分 P 覆盖缺失值、类型转换、分箱和异常值；视频没有单独讲重复值与字符串清洗，这两部分请以本章正文和练习脚本为主。

    - [P44 缺失值的处理](https://www.bilibili.com/video/BV1D9GLzyEL6/?p=44)
    - [P45 数据类型转换](https://www.bilibili.com/video/BV1D9GLzyEL6/?p=45)
    - [P47 数据分箱](https://www.bilibili.com/video/BV1D9GLzyEL6/?p=47)
    - [P64 项目实战：数据类型转换](https://www.bilibili.com/video/BV1D9GLzyEL6/?p=64)
    - [P65 项目实战：异常值处理](https://www.bilibili.com/video/BV1D9GLzyEL6/?p=65)
...
!!! note "本站补充"
    下面的完整脚本对应本章 7.1～7.5，练习缺失值、重复值、异常值、字符串清洗和分类数据转换，并补充现代 Pandas 的处理建议。

    直接运行会依次学习整章；也可用 `--list` 查看路线、用 `--section 7.3` 专练异常值，或用 `--skip-exercises` 先跳过章末练习。脚本强调保留原始数据、记录清洗规则和比较清洗前后质量。

[在线分节查看](../../code-lab.md?chapter=7){ .md-button .md-button--primary }
[下载第 7 章 Python 脚本](../../assets/code/chapter07_pandas_cleaning_tutorial.py){ .md-button download }
